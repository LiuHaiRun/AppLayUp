...
public class TeamQuenAdapter extends RecyclerView.Adapter {
    ...
    private List<RankingData.ContentBeanX.RoundsBean.ContentBean.DataBean.DataBean2> teamList;
    private List<RankingData.ContentBeanX.RoundsBean.ContentBean.DataBean> cbaList;
    private String teamPos;
    private String fatherTab;
    private final int TITLE_TYPE = 1;
    private final int NORMAL_TYPE = 2;

    public TeamQuenAdapter(Context pContext, List<RankingData.ContentBeanX.RoundsBean.ContentBean.DataBean.DataBean2> pTeamList, String pTeamPos) {
        mContext = pContext;
        teamList = pTeamList;
        this.teamPos = pTeamPos;
    }

    public TeamQuenAdapter(Context pContext,String fatherTab, List<RankingData.ContentBeanX.RoundsBean.ContentBean.DataBean> cbaList) {
        mContext = pContext;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == NORMAL_TYPE){
            return  new ViewHolder(View.inflate(mContext, R.layout.team_quen_layout, null));
        }else {
            return new TitleHolder(View.inflate(mContext, R.layout.team_title_layout, null));
        }
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (getItemViewType(position) == NORMAL_TYPE) {
            ...
            if (!TextUtils.isEmpty(fatherTab) && fatherTab.equals("CBA")){
                RankingData.ContentBeanX.RoundsBean.ContentBean.DataBean cbaData = cbaList.get(position - 1);
                normalHolder.num.setText(cbaData.rank);
                Glide.with(mContext).load(cbaData.team_logo).dontAnimate().into(normalHolder.teamLogo);
                ...
                RankingData.ContentBeanX.RoundsBean.ContentBean.DataBean.DataBean2 dataBean2 = teamList.get(position - 1);
                normalHolder.num.setText(dataBean2.rank);
                Glide.with(mContext).load(dataBean2.team_logo).dontAnimate().into(normalHolder.teamLogo);
